...
namespace TheInfiniteCrusade.Backend.NewComponents.ActionTriggers
{
    ...
    class ActionOnKill : UnitFactComponentDelegate, IInitiatorRulebookHandler<RuleAttackWithWeapon>, IRulebookHandler<RuleAttackWithWeapon>, ISubscriber, IInitiatorRulebookSubscriber
    {
        public ConditionsChecker attackerConditions;
        public ConditionsChecker targetConditions;
        public bool RequiresWorthy;
        public bool RequiresWeaponFocus;
        public bool RequiresStrike;
        ...
        public void OnEventDidTrigger(RuleAttackWithWeapon evt)
        {
            if (evt.AttackRoll.IsCriticalConfirmed)
            {
                ...
                if (RequiresStrike)
                {
                    if (evt.Reason.Ability?.Blueprint.Components.OfType<ManeuverInformation>().Any(x => x.ManeuverType == ManeuverType.Strike) != true)
                        return;
                }
